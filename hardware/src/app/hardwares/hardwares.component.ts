...
export class HardwaresComponent implements OnInit {
  ...
  constructor(
    ...
    this.isUserDeleter = this.authenticationService.isUserDeleter();
  }
  ...
  add(code: string, name: string, type: string, availableNum: number, price: number): void {
    code = code.trim();
    name = name.trim();
    type = type.trim();
    if (!code || !name || !type) {
    ...
    this.hardwareService.addHardware({code, name, type, availableNum, price} as Hardware)
      .subscribe(
        hardware => {
